...
Given 
	...
	\begin{itemize}
		...
		\item<only@2> \alert{cone} $\cc \subset \cw^{2,\infty}$ defined by constants $n_{\init}$ and $\fC_0$
	\end{itemize}
		\vspace{-3ex}
	\only<1>{one may \alert{nonadaptively}}\only<2>{we \alert{adaptively}} construct a linear spline, $S(f,x_{0:n})$ with $a = x_0 <x_1 < \cdots < x_n = b$, satisfying
	\begin{gather*}
...
We will construct our \alert{adaptive} spline algorithm for functions in 
	...
	\vspace{-5.5ex}	
		\begin{gather*}
		\cc =  \Bigl\{f : \norm[{[\alpha,\gamma]}]{f''} \le \max \bigl (B_{\pm}(f'',\alpha,\gamma)  \bigr), \ \gamma - \alpha \le h_{\pm} < \fh\Bigr\} \\[-1ex]
		B_{-}(f'',\alpha,\gamma) = \begin{cases} \fC(h_{-}) \norm[{-\infty,[\gamma - h_-, \alpha]}]{f''}, & a \le \gamma - h_- \\
